	...
	try
		tell application "System Events"
			tell process appName
				set getItem to enabled of menu item theMenuItemName of menu theMenuName of menu bar 1
				if getItem is true then
					click menu item theMenuItemName of menu theMenuName of menu bar 1
					set activeMenuItem to enabled of menu item subToolName of menu of menu item theSubMenuItem of menu theMenuItemName of menu bar 1
					...
					errorMessage(theMenuItemName & "Please click into a text frame and try again.")
					return false
				end if
			end tell
		end tell
	on error
		errorMessage(theMenuItemName & " - " & theSubMenuItem & " - " & subToolName & " wasn't able to be selected.\n\nPlease click into a text frame and try again.")
		return false
	end try
end subMenuItem

subMenuItem("Tools", "Text", "Inserts", "Performance Time")